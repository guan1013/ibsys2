...
public class PlanungstoolGUI {
	private static ResourceBundle BUNDLE = ResourceBundle.getBundle(
			"de.hska.centurion.gui.i18n.messages", new Locale("de")); //$NON-NLS-1$
	private static ResourceBundle BUNDLE2 = ResourceBundle.getBundle(
			"de.hska.centurion.gui.i18n.messages", new Locale("en")); //$NON-NLS-1$
	private static ResourceBundle BUNDLE3 = ResourceBundle.getBundle(
			"de.hska.centurion.gui.i18n.messages", new Locale("ru")); //$NON-NLS-1$
		...
		if (exception.getMessage() != null) {
			completeMessage += "\n"
					+ exception.getMessage().substring(0,
							Math.min(exception.getMessage().length(), 100));
		}

		JOptionPane.showMessageDialog(parent, completeMessage, "Fehler",
	...
	public PlanungstoolGUI(String[] args) {
		...
		BUNDLE = ResourceBundle.getBundle(
				"de.hska.centurion.gui.i18n.messages", new Locale(locale)); //$NON-NLS-1$
	...
	private void calculateCapacity() {
		try {
			...
			String message = "Kapazit�t an Arbeitsplatz " + e.getWorkplace()
					+ " ist nicht ausreichend f�r " + e.getShifts()
					+ " Schichten und " + e.getOvertime().intValue()
					+ " Minuten �berstunden.";

			showErrorDialog(message, e, frameMain);
		}
	}
	...
	private void calculateSafetyStock() {
		...
		Map<String, Integer> safetyStockGui;
		try {
			safetyStockGui = productionService.calculateProduction(userInput
					.getSafetyStock());

			// Display calculation result on gui
			displaySafetyStock(safetyStockGui);
			
		} catch (InefficientProductionException e) {
			String message = "Das Produkt " + e.getItem()
					+ " hat eine ineffiziente Fixkosten Deckung von "
					+ e.getFixCostCoverage() + ". Daher werden von "
					+ e.getItem() + " keine Produziert.";
			showErrorDialog(message, e, frameMain);

			userInput.getSafetyStock().changeStock(e.getItem(), 0);

			if (e.getItem().equalsIgnoreCase("P1")) {
				userInput.getSales().setChildrenSales(0);
				userInput.getDirectSales().setChildrenSales(0);
			} else if (e.getItem().equalsIgnoreCase("P2")) {
				userInput.getSales().setWomenSales(0);
				userInput.getDirectSales().setWomenSales(0);
			} else if (e.getItem().equalsIgnoreCase("P3")) {
				userInput.getSales().setMenSales(0);
				userInput.getDirectSales().setMenSales(0);
			}
	...
	private void initialize() {
		...
		tableWaitingListStock.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "Missing Part", "Period", "Production Order",
						"Batch", "Item", "Quantity" }) {
		...
		spinnerStep2P1DirectSales.setModel(new SpinnerNumberModel(
				new Integer(0), new Integer(0), null, new Integer(10)));
		...
		spinnerStep5ShiftWorkplace1
				.setModel(new SpinnerNumberModel(0, 0, 3, 1));
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					try {
						...
						ex.printStackTrace();
						showErrorDialog(
								"XML-Datei konnte nicht geladen werden!", ex,
								frameMain);
						return;
					}
					...
					if (orders != null) {
						for (Order order : orders) {
							model.addRow(new Object[] {
									...
									order.getArticle(), order.getAmount(),
									order.getInwardStockMovementAvg(),
									order.getInwardStockMovementMax() });
						}
					}
					...
					// Bisherige angezeigte Daten entfernen
					model = (DefaultTableModel) tableWaitingListStock
							.getModel();
					removeAllRowsFromTable(model);

					// Alle Wartelisten anzeigen
					List<MissingPart> partsWaiting = results
							.getWaitingListStock().getMissingParts();
					for (MissingPart mp : partsWaiting) {
						...
						if (mp.getWaitingList() != null) {
							...
							for (WaitingList waitingList : mp.getWaitingList()) {
								...
								model.addRow(new Object[] {
										mp.getId(),
										waitingList.getPeriod(),
										waitingList.getOrder(),
										waitingList.getFirstBatch() + "-"
												+ waitingList.getLastBatch(),
										waitingList.getItem(),
										waitingList.getAmount() });
							}
						} else {
